...
#[AsCommand(name: 'app:comments:status', description: 'Return the current status and details of a comment.')]
final class GetCommentStatusCommand extends Command
{
    use HandleTrait;
    use JsonOutputTrait;
    ...
    {
        $result = $this->handle($this->messageBus, new GetCommentQuery($this->commentId($input)));
        if (!$result instanceof CommentView) {
            throw new \LogicException('Unexpected comment view.');
        }
